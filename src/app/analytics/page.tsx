...
const deptBarData = departmentStats.map((d) => ({
    name: d.department,
    Points: Math.round(d.totalPoints / 100),
    ...
    return (
        <DashboardLayout user={currentStudent} role="student">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
                ...
                <StatCard title="Active Students" value="1,200" icon={Users} color="purple" />
            </div>
                ...
                <TabsContent value="departments" className="space-y-5">
                    <ChartWrapper title="Department Performance" description="Achievement points and student count by department">
                        <CustomBarChart
                            data={deptBarData}
                            bars={[
                                { key: "Points", label: "Points (×100)", color: "#2563eb" },
                                        ...
                                        {departmentStats.map((d) => (
                                            <tr key={d.department} className="hover:bg-slate-50">
                                                ...
                                                <td className="px-4 py-2.5 text-blue-700">
                                                    {Math.round((d.approvedAchievements / d.totalAchievements) * 100)}%
                                                </td>
